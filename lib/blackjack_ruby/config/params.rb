  ...
  module Config
    class Params < SimpleDelegator
      ...
      def require(*entries)
        ...
        raise "Raise missing entries: #{missing_entries}"
      end
      ...
      def require_one_of(*keys)
        ...
        unless keys.any?
          ...
          raise "Missing parameters: #{message}"
        end
      end
      ...
      def permit(*filters)
        ...
        raise "Parameters are not permitted: #{non_permitted_params.to_s}" unless non_permitted_params.empty?
      end
      ...
      def permit_value(key, values)
        ...
        unless !value || values.include?(value)
          raise "Parameter value is not permitted '#{key}': #{value}"
        end
      end
